...
def r_loss(y_true, y_pred): #[batch_size x 100 x 3] -> [batch_size]
    ...
    for i in range(parts):        
        ...
        expand_inputs  = tf.expand_dims(t_true_keep_R_conc, 1)
        expand_outputs = tf.expand_dims(t_pred_keep_R_conc, 0)
...
model.compile(optimizer=opt, loss = r_loss)#, metrics=["accuracy"])
