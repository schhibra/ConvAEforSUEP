...
def FocalLoss(y_true, y_pred, alpha=ALPHA, gamma=GAMMA):
    ...
    for i in range(parts):
        ...
        focal.append(K.sum(K.square(y_true_tmp - y_pred_tmp), axis = [0]))
    return focal
...
#model.compile(optimizer=opt, loss = FocalLoss, metrics=[sum_y_true, sum_y_pred, 'accuracy'])
model.compile(optimizer=opt, loss = BinaryFocalLoss(gamma=2), metrics=[sum_y_true, sum_y_pred, 'accuracy'])

version = "v2_1_Focalloss_linear_relu"
